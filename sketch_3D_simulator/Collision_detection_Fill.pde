class collision {
  ...
  void Fill(float[][] contour, float[][] foot) {
    ...
    for (int j = 0; j < 1500/dx; j++) {
      ...
      if (IPoints.length > 1) {
        ...
        if (IPoints.length > 7) {
          line(x, IPoints[0], currentLayerDraw, x, IPoints[1], currentLayerDraw);
          line(x, IPoints[2], currentLayerDraw, x, IPoints[3], currentLayerDraw);
          line(x, IPoints[4], currentLayerDraw, x, IPoints[5], currentLayerDraw);
          line(x, IPoints[6], currentLayerDraw, x, IPoints[7], currentLayerDraw);
    ...
    currentLayerDraw = currentLayerDraw + layerHeight;
  } // end void
  ...
  void FillWrite(float[][] contour, float[][] foot) {
    ...
    for (int j = 0; j < 1500/dx; j++) {
      ...
      IPoints = sort(IPoints);
      if (IPoints.length > 1 && IPoints[0] > 0 && IPoints[1] > 0) {
        if (IPoints.length > 7) {
