class contour {
  ...
  void Coord(float[] CX, float[]CY) {
    ...
    for (int i = 0; i < CX.length; i++) {
      coPoints[nrCoPoints  ][0] = s*(CX[i] + 250 + xplace);
      coPoints[nrCoPoints++][1] = s*(CY[i] + 185 + yplace);
    }
    ...
    //Calculate X and Y center of the figure for rotation
  ...
  /*void contourDraw() {
    ...
    for (int i=1; i < numSteps; i++) {
      ...
      //draw the contour
      stroke(0, 128, 0);
      line(startX, startY, currentLayerDraw, endX, endY, currentLayerDraw);
      startX = endX;
      startY = endY;
    }
