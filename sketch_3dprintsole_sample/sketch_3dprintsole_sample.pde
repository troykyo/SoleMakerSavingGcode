/* ©Niek van Sleeuwen TU/e 2016
 ...
 SVG files are extracted from a STL file, every SVG file is a 0.4mm layer.
 *********************************************************************************************
...
int dx = 4; //per dx pixels a line is drawn to fill the insole.
...
//For 3d printing purposes, Ultimaker 2 baseplate is not big enough to print the entire sole.
//*****************************************MIN VALUE 0 *********************************
int Xmin = int(0*DPMM); // defines start X postion to print, cuts off everything before this X coordinate
//*****************************MAX VALUE width of screen, prints entire sole at int(272*DPMM)*************************
//Xmin added to adjust the Xwidth to the coordinates, as the entire drawing translates. You can measure the parts in mm and fill them in here
int Xwidth = int(275*DPMM) - Xmin; // defines the max X, cuts off everything after this X coordinate
...
void setup() {
  ...
  Date now = new Date();
  fileName = "SOLE_TROY" + sdf.format(now) + "_ACCU_"+ numSteps + "points_DX_" + dx + "_SCALE_"+ s + "_StartX_" + truncate(Xmin/DPMM)  + "mm_EndX_" + truncate(Xwidth/DPMM) + "mm.gcode";
