class contour {
  ...
  void Coord(float[] CX, float[]CY) {
    ...
    for (int i = 0; i < CX.length; i++) {
      coPoints[nrCoPoints  ][0] = s*(CX[i] + 230 - Xmin);
      coPoints[nrCoPoints++][1] = s*(CY[i] + 180);
    }
  ...
  void contourWrite() {
    ...
    for (int i=1; i < numSteps; i++) {
      float endX = contourCurve[i][0];
      float endY = contourCurve[i][1];
      if ((startX < Xwidth && startX > Xmin)|| (endX < Xwidth && endX > Xmin)) {
    ...
    for (int j = 0; j < 1000/dx; j++) {
      ...
      IPoints = sort(IPoints);
      if (IPoints.length > 1) {
        if (x < Xwidth && x > Xmin && IPoints[0] > 0 && IPoints[1] > 0) {
